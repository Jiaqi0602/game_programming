 ...
 * @author Joyen
 */
public class MCTreeSearchNode {

	MCTreeSearchState state;
	MCTreeSearchNode parent;
	MOVE move;
	int visited;
	float value;
	List<MCTreeSearchNode> children;
	int directions;
	int time;
	...
	public MCTreeSearchNode(MCTreeSearchState state, MCTreeSearchNode parent, MOVE move, int time) {
		super();
		this.state = state;
		this.parent = parent;
		this.move = move;
		...
		this.value = 0;
		this.children = new ArrayList<MCTreeSearchNode>();
		this.time = time;
		this.simulations = new ArrayList<Integer>();
	}
	

	public MCTreeSearchNode expand() {
		...
		if (junction != -1){
			...
			MCTreeSearchState childState = runExperimentUntilJunction(new AggressiveGhosts(), state.getGame(), junction, nextMove);
			...
			MCTreeSearchNode child = new MCTreeSearchNode(childState, this, nextMove, time + distance);
			children.add(child);
			return child;
		}
	...
	private MCTreeSearchState runExperimentUntilJunction(Controller<EnumMap<GHOST,MOVE>> ghostController, Game game, int junction, MOVE move) {
		...
		*/
		...
		while(now != junction){
			...
			if (livesNow < livesBefore)
				return new MCTreeSearchState(false, clone);
		...
		return new MCTreeSearchState(true, clone);
	...
	private int closestJunction(MOVE move) {
		...
		while(!PacManMCTreeSearch.junctions.contains(current) || current == from){
	...
	public MCTreeSearchNode getParent() {
		return parent;
	}

	public void setParent(MCTreeSearchNode parent) {
		this.parent = parent;
	}

	public MCTreeSearchState getState() {
		return state;
	}

	public void setState(MCTreeSearchState state) {
		this.state = state;
	}
	...
	public List<MCTreeSearchNode> getChildren() {
		return children;
	}

	public void setChildren(List<MCTreeSearchNode> children) {
	...
	public String print(int level) {
		...
		*/
		...
		int next = level+1;
		for(MCTreeSearchNode child : children){
