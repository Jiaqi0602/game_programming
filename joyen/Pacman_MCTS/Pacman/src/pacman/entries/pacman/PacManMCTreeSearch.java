...
public class PacManMCTreeSearch extends Controller<MOVE>{
	...
	private MOVE MctsSearch(Game game, long ms) {
		...
		MCTreeSearchNode v0 = new MCTreeSearchNode(new MCTreeSearchState(true, game), null, game.getPacmanLastMoveMade(), 0);
		
		while(new Date().getTime() < start + ms){
			
			MCTreeSearchNode v1 = treePolicy(v0);
		...
		MCTreeSearchNode bestNode = bestChild(v0, 0);
		MOVE move = MOVE.UP;
		if (bestNode != null)
	...
	private MCTreeSearchNode treePolicy(MCTreeSearchNode node) {
	...
	private MCTreeSearchNode bestChild(MCTreeSearchNode v, float c) {
		
		float bestValue = -99999999;
		MCTreeSearchNode urgent = null;
		
		for(MCTreeSearchNode node : v.children){
	...
	private boolean dieTest(MCTreeSearchNode v, MCTreeSearchNode node) {
		
		Controller<MOVE> pacManController = new RandomJunc();
	...
	private float UCT(MCTreeSearchNode node, float c) {
	...
	private int defaultPolicy(MCTreeSearchNode node, MCTreeSearchNode root) {
	...
	private void backup(MCTreeSearchNode v, int score) {
	...
	public int runExperimentWithAvgScoreLimit(MCTreeSearchNode node, int steps) {
